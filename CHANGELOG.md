...
### Run spend counters and count ceilings (fixes #92)

- **`set_step` (every `hpr run step --status done`) and `hpr run reconcile` fold vault_tag-filtered notes/sources into `spend`**. Pipeline agents rarely call `hpr run spend`, so a completed run used to report all-zero counters. Counts come from the vault DB by tag (`notes` joined on `tags.tag = vault_tag`) and each note's `raw_file` frontmatter — not mtime, which concurrent runs and `note update` / `repair` / `sources score` rewrite.
- **`hpr run init --max-sources` / `--max-notes`** are the observable ceilings this repo can actually measure. Crossing them at a step boundary blocks the run (`blocked_on` = `max_sources` / `max_notes`). Agent-reported `estimated_usd` is unchanged; this package does not carry unit prices.
- **`hpr run step --status done --force`** clears a count block so resume can proceed when no new files landed. Force-clears are counted (`counts_force_cleared`, cap 3) so a resume cannot spin.
