...
- **`anysearch` web provider.** Anonymous or API-key search with page extraction, excerpt fallback for unavailable pages, and an 8,000-character extraction limit. Opt-in via `[web] provider = "anysearch"`; the provider's Python interface also supports vertical capability discovery and parallel batch search without a new dependency.
- **Install API-backed vaults without Chromium.** Installing a vault with an API provider now skips browser setup; `install --skip-browser` also allows explicit browser-free setup.
